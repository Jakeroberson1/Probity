...
A fixed, versioned procedure for predicting FDA catalyst outcomes. AdCom votes first, then PDUFAs, then readouts. Every brief runs the same 7 gates in the same order. Every gate is scored with cited evidence. The verdict is a weighted aggregate published as a probability, not a hot take. When we're wrong, we point to exactly which gate failed — and fix it in the next version. That public error log is the product.

### The 7 gates (run in order)

**Gate 1 — Efficacy (weight 25).** Does it work?
...
**Gate 2 — Safety (weight 20).** What are the harms?
...
**Gate 3 — Precedent (weight 15).** What has FDA done before in this situation?
- First-of-kind products face a higher bar. Find the closest prior panel votes and final FDA decisions. **Weight committee-specific precedent heaviest** (which of the 33 committees is hearing this matters — each has its own culture and voting patterns). If FDA broke precedent before, note exactly why. No precedent found = score Toss-up, never guess.

**Gate 4 — Panel (weight 10).** Who's voting?
...
**Gate 5 — The tell (weight 20).** What is the FDA worried about?
...
**Gate 6 — Unmet need (weight 5).** How badly is this wanted?
...
**Gate 7 — Commercial reality (weight 5).** If it wins, does the world change?
...
Each gate: Strong Yes (+2) / Lean Yes (+1) / Toss-up (0) / Lean No (−1) / Strong No (−2). Weighted sum → total (−200 to +200) → published probability via bands (v1.0 priors, calibrated after 10 events):

| Total score | Published probability of YES |
|---|---|
| +80 and up | 75–90% |
| +30 to +79 | 60–74% |
| −29 to +29 | 45–59% (true coin flip — say so) |
| −79 to −30 | 26–44% |
| −80 and down | 10–25% |

Rules:
- Binary call follows the probability (≥50% = YES). Never publish a binary call without the probability next to it.
- A Toss-up on Gate 5 caps the published probability at 65%. If you can't read the FDA, you don't get to be confident.
- Every gate score cites ≥1 source. No citable evidence = Toss-up, never vibes.

### Analyst override

Jake may override any gate score or the final probability — but must state *which* gate, changed from/to what, and *why*, in the published brief. Overrides are logged and graded. If overrides consistently beat the machine, the weights change.
